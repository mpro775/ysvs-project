...
import { useTermsAndConditionsPage } from '@/api/hooks/useContent';

export default function TermsPage() {
  const { data, isLoading, isError } = useTermsAndConditionsPage();
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      {isLoading ? (
        ...
        <>
          <h1 className="text-3xl font-bold">{data?.titleAr || 'الشروط والأحكام'}</h1>
          {data?.effectiveDate && (
      ...
      {isError && (
        <Alert variant="destructive" className="mt-6">
          <AlertTitle>تعذر تحميل الشروط والأحكام</AlertTitle>
          <AlertDescription>
